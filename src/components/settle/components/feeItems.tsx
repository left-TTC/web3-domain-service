...
const FeeItems: React.FC<FeeItemsProps> = ({
    ...
    return(
        <div className="bg-black/40 rounded-xl py-2 px-4 md:py-4 border border-white/5 space-y-2">
            {feeItems.map((item, index) => (
                <div key={index} className="flex justify-between items-center text-[12px] md:text-sm">
                    <div className="flex font-normal items-center gap-1 md:gap-1.5 text-gray-400 group relative">
                        {item.label}
                        {item.info && (
                        <>
                            <Info size={13} className="hover:text-[#B4FC75]" />
            ...
            <div className="h-px bg-white/10 my-1 md:my-2"/>
            
            <div className="flex justify-between items-center">
                <span className="text-[12px] md:text-[14px] text-white font-bold">Total</span>
                <span className="text-[13px] md:text-[16px] font-mono font-bold text-[#B4FC75]">{(total/1e9).toFixed(4)} SOL</span>
            </div>
        </div>
    )
}
